...
function initCheckout() {
  ...
  // Toggle Checkout Items Summary Dropdown
  const summaryToggle = document.getElementById('summary-toggle-btn');
  const summaryItemsList = document.getElementById('checkout-summary-items-list');
  const summaryToggleIcon = document.getElementById('summary-toggle-icon');
  
  if (summaryToggle && summaryItemsList && summaryToggleIcon) {
    summaryToggle.addEventListener('click', () => {
      summaryItemsList.classList.toggle('open');
  ...
  if (copyBtn) {
    copyBtn.addEventListener('click', () => {
      ...
      if (numElement) {
        ...
        showToast('NUMBER COPIED', 'Recipient phone number 01606590420 copied.');
        
        // Visual button state transition
        const copyBtnSpan = copyBtn.querySelector('span');
        const copyBtnIcon = copyBtn.querySelector('i');
        if (copyBtnSpan) copyBtnSpan.innerText = 'COPIED!';
        if (copyBtnIcon) copyBtnIcon.className = 'bi bi-check-lg';
        copyBtn.classList.add('copied');
        
        setTimeout(() => {
          if (copyBtnSpan) copyBtnSpan.innerText = 'COPY';
          if (copyBtnIcon) copyBtnIcon.className = 'bi bi-copy';
  ...
  // Step mobile wallet selectors (bKash & Nagad selectors)
  paymentOptions.forEach(opt => {
    opt.addEventListener('click', (e) => {
      ...
      // Update Recipient Card Container Theme
      const walletCard = document.getElementById('wallet-card-container');
      if (walletCard) {
        walletCard.className = `wallet-payment-card ${method}-theme`;
      }
      
      // Update Form inputs Theme (for glowing borders and icon accents)
      const formBox = document.getElementById('payment-form-inputs');
      if (formBox) {
        formBox.className = `payment-form-box active ${method}-active-fields`;
      }
      
      // Update label in inputs
      const senderLabel = document.getElementById('sender-number-label');
      if (senderLabel) {
        senderLabel.innerHTML = `Sender ${method === 'bkash' ? 'bKash' : 'Nagad'} Wallet Number / আপনার ${method === 'bkash' ? 'বিকাশ' : 'নগদ'} নাম্বার`;
      }
...
function updateCheckoutTotals() {
  ...
  document.getElementById('checkout-subtotal-val').innerHTML = `$${subtotal.toFixed(2)} USD`;
  document.getElementById('checkout-shipping-val').innerHTML = `৳${chargeAmount}.00 BDT`;
  document.getElementById('checkout-total-val').innerHTML = `$${subtotal.toFixed(2)} USD`; // Subtotal remains COD
  ...
  // Update checkout order summary pane list dynamically
  const summaryList = document.getElementById('checkout-summary-items-list');
  if (summaryList) {
    if (state.cart.length === 0) {
      summaryList.innerHTML = `<div style="text-align: center; color: var(--text-muted); font-size: 0.8rem; padding: 20px;">Your cart is empty</div>`;
    } else {
      summaryList.innerHTML = state.cart.map(item => `
        <div class="checkout-summary-item">
          <div class="checkout-summary-item-left">
            <img src="${item.product.image}" alt="${item.product.name}" class="checkout-summary-item-img">
            <div class="checkout-summary-item-info">
              <span class="checkout-summary-item-name">${item.product.name}</span>
              <span class="checkout-summary-item-meta">Size: ${item.size} | Qty: ${item.quantity}</span>
            </div>
          </div>
          <span class="checkout-summary-item-price">$${(item.product.price * item.quantity).toFixed(2)}</span>
        </div>
      `).join('');
    }
  }
...
function resetCheckoutForm() {
  ...
  // Reset items summary collapse state
  const summaryList = document.getElementById('checkout-summary-items-list');
  if (summaryList) summaryList.classList.remove('open');
  const summaryToggleIcon = document.getElementById('summary-toggle-icon');
  if (summaryToggleIcon) summaryToggleIcon.classList.remove('open');
  
  // Reset recipient card class theme to default bKash
  const walletCard = document.getElementById('wallet-card-container');
  if (walletCard) {
    walletCard.className = `wallet-payment-card bkash-theme`;
  }
  
  // Reset payment form box inputs theme to default bKash
  const formBox = document.getElementById('payment-form-inputs');
  if (formBox) {
    formBox.className = `payment-form-box active bkash-active-fields`;
  }
  
  // Set wallet selector active to bKash
  const paymentOptions = document.querySelectorAll('.checkout-drawer .payment-option');
  paymentOptions.forEach(o => o.classList.remove('active'));
  if (paymentOptions.length > 0) paymentOptions[0].classList.add('active');
  
  // Set label translation
  const senderLabel = document.getElementById('sender-number-label');
  if (senderLabel) senderLabel.innerHTML = 'Sender bKash Wallet Number / আপনার বিকাশ নাম্বার';
  ...
  // Reset input fields
  document.querySelectorAll('.checkout-drawer .form-input').forEach(input => {
...
function progressCheckout() {
  ...
  if (state.checkoutStep === 1) {
    ...
    // Populate step 3 success tracking variables dynamically
    const orderIdEl = document.getElementById('success-order-id');
    if (orderIdEl) {
      orderIdEl.innerText = `FTZ-${Math.floor(10000 + Math.random() * 90000)}`;
    }
    const walletUsedEl = document.getElementById('success-wallet-used');
    if (walletUsedEl) {
      const methodLabel = state.paymentMethod === 'bkash' ? 'bKash Personal Wallet' : 'Nagad Personal Wallet';
      walletUsedEl.innerText = `${methodLabel} (${senderNum})`;
    }
